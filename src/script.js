...
const ringInnerRadius = 1.275 * saturnScale;
const ringOuterRadius = 1.375 * saturnScale;
...
const smallInnerRingTexture = textureLoader.load(
  "/textures/saturnringcolor.jpg"
);
...
const smallInnerRing = new THREE.Mesh(
  smallInnerRingGeometry,
  smallInnerRingMaterial
);
scene.add(smallInnerRing);
// Big outer ring
const bigOuterRingGeometry = new THREE.TorusGeometry(
  ringOuterRadius,
  0.09,
  2,
  100
...
const titanDistance = 12.2185 * saturnScale; //scaled
...
const dioneDistance = 3.774 * saturnScale;
...
const rheaDistance = 5.2704 * saturnScale;
...
rheaMesh.name = "rheaMesh";
scene.add(rheaMesh);
// Iapetus
const iapetusRadius = 0.0735 * saturnScale; //scaled
const iapetusDistance = 3.5613 * saturnScale;
...
iapetusMesh.name = "iapetusMesh";
scene.add(iapetusMesh);
// Mimas
const mimasRadius = 0.0396 * saturnScale; //scaled
const mimasDistance = 1.8552 * saturnScale;
...
const enceladusDistance = 2.3802 * saturnScale;
...
enceladusMesh.name = "enceladusMesh";
scene.add(enceladusMesh);
// TETHYS
const tethysRadius = 0.0531 * saturnScale; //scaled
const tethysDistance = 2.9466 * saturnScale;
...
hyperionMesh.name = "hyperionMesh";
...
const marsRadius = 0.5; // Saturn radius scaled
...
const earthRadius = 0.6; // Saturn radius scaled
...
function updateLabels() {
  for (const { object, labelDiv } of labels) {
    ...
    labelDiv.style.top =
      (-screenPosition.y * sizes.height) / 2 + sizes.height / 2 + "px";
  }
}
...
function showInfoCard(info) {
  ...
  card.innerHTML += `
    ...
    <div class="rotation">
      <h5>Rotation Periond:</h5>
      <p>${info.rotationPeriod}</p>
    </div>
...
sun.castShadow = true;
sun.add(
  new THREE.Mesh(sphereLight, new THREE.MeshBasicMaterial({ color: "#fcf3db" }))
);
scene.add(sun);
...
const radiansPerSecond =
  (2 * Math.PI) / (saturnRotationPeriod * secondsPerHour);
//  Titan's orbit
...
const tick = () => {
  ...
  const mimasScaleRotation = mimasRotation / 1000000000;
  ...
  const enceladusScaleRotation = enceladusRotation / 1000000000;
  ...
  const tethysScaleRotation = tethysRotation / 1000000000;
  ...
  const hyperionScaleRotation = hyperionRotation / 10000000000;
